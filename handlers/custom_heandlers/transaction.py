...
from keyboards.inline_keyboards import (
    ...
    transaction_save_kb,
    save_category_kb, change_transaction_details_kb,
)
...
@router.callback_query(UserState.change_transaction_details, Text('change_transaction_date'))
...
@router.callback_query(UserState.change_transaction_details, simple_cal_callback.filter())
...
@router.callback_query(UserState.transaction_category, Text(startswith='transaction_category:'))
async def transaction_callback_description(callback: CallbackQuery, state: FSMContext):
    """
    Функция. Уточненяем описание операции.
    """
    try:
        logger.debug(f'Пользователь {callback.message.chat.id}. Уточняем описание операции')
...
async def callback_change_unwritten_category(callback: CallbackQuery, state: FSMContext):
    """
    Функция. Уточняем перед записью какие данные изменить
    """
    logger.debug(f'Пользователь {callback.message.chat.id} - '
...
@router.callback_query(UserState.change_transaction_details, Text('change_transaction_summ'))
async def callback_change_unwritten_category(callback: CallbackQuery, state: FSMContext):
    """
...
@router.callback_query(UserState.change_transaction_details, Text('change_transaction_descr'))
